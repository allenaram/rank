...
def process_command_args():
    ...
    parser.add_argument('--pretrain_models_path', type=str,
                        default=rootPath + "/experiments/tid2013/rankiqa/" + 'model.ckpt-89999')

                        # default=rootPath + "/experiments/rankiqa/" + 'model.ckpt-6999_ft')
...
def get_image_list(args):
    ...
    for line in f:
        ...
        train_image_paths.append(os.path.join(BASE_PATH,'distorted_images',image_path))
    ...
    for line in f:
        image_score,image_path = line.strip("\n").split()[0],line.strip("\n").split()[1].lower()
        test_image_paths.append(os.path.join(BASE_PATH,'distorted_images',image_path))
...
def train(args):
    ...
    with graph.as_default():
        ...
        summary_writer = tf.summary.FileWriter(os.path.join(args.logs_dir, 'iqa_train'))
        summary_test = tf.summary.FileWriter(os.path.join(args.logs_dir, 'iqa_test'))
...
if args.dataset == 'tid2013':
    args.train_list = os.path.join(BASE_PATH,"tid2013_train.txt")
    args.test_list = os.path.join(BASE_PATH,"tid2013_test.txt")
...
output_dir = os.path.join(args.ckpt_dir, args.dataset) # experiments\tid2013
args.data_dir = os.path.join(args.data_dir, args.dataset) # E:\外包服务\质量评价毕设\前期代码\tid2013\tid2013
args.ckpt_dir = os.path.join(args.ckpt_dir, args.dataset, args.exp_name) # experiments\tid2013\finetuneiqav2
args.logs_dir = os.path.join(args.logs_dir, args.dataset, "logs") # experiments\tid2013\logs
